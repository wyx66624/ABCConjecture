...
The previous modules provide actual primitive root and split-square
`ABCPoint`s, a fixed `log 2` radical-overlap bound, and the exact scalar
root-transfer threshold.  This file combines them.

Every split-square abc violation either descends to an `epsilon/2` violation
for the primitive root triple, with an explicit constant, or forces
height-scale logarithmic multiplicity excess in the companion sum `s=y+x`.
