...
then the small endpoint factors as

`m = (y-x)(y+x)`.

The short-gap threshold descends from the endpoint height `2*log y` to the
root gap `log(y-x)`: if

`(1+epsilon) * log m < (2+epsilon) * log y`,

and `log(y-x)+log y <= log m`, then

`(1+epsilon) * log(y-x) < log y`.

This file records the exact algebraic and scalar descent.  It assumes no abc
estimate.
-/
...
/-- The descended exponent is exactly `1/(1+epsilon)` in denominator-free
form; no loss from the original short-gap threshold remains. -/
theorem rootGap_powerSaving_positive
  ...
  · have hypos : 0 < hy := by
      nlinarith
    linarith
  · have hTdpos : 0 < Td := lt_of_not_ge hTd
    nlinarith
