...
scalar model satisfying the abc-violation budget, the non-short-gap lower
bound, the two large-endpoint lower bounds, and all three fifth-power residue
budgets, while all three fifth-root weights are zero.
-/
...
/-- Positivity of `epsilon` alone cannot force a positive fifth-root weight
from the aggregate scalar hypotheses. -/
theorem not_every_aggregate_model_has_positive_fifthRoot
  ...
  obtain ⟨h, Tm, TM, Tc, rm, rM, rc, qm, qM, qc,
    hh, hv, hm, hM, hc, hmq, hMq, hcq, hqm, hqM, hqc⟩ :=
    exists_zero_fifthRoot_aggregate_model hepsilon hcritical
  have hpositive := hall h Tm TM Tc rm rM rc qm qM qc
    hh hv hm hM hc hmq hMq hcq
  rw [hqm, hqM, hqc] at hpositive
