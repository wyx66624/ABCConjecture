...
For fixed `M` and `0 < epsilon < 1/4`, this module constructs constants
...
theorem complete_genEllLemma41
  ...
  have hNlowerY : (Nlower : ℝ) ≤ y := by
    have hNN : (Nlower : ℝ) ≤ xε := hxNlowerReal.le
    linarith
  have hNlowerX :
