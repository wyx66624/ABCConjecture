...
with `A` and `B` squarefree, a prime shared by `R` and `A` occurs in the left
endpoint `R*A` with exponent exactly `v_p(R)+1`.  Consequently its exact
p-adic contact depth in `S*B-m` is `v_p(R)+1`.  The right side is symmetric.

This identifies the two modulus exponent vectors with the excess p-adic
contact depths of the opposite endpoint against the gap.  It introduces no
height estimate or abc hypothesis.
-/
...
/-- Multiplying by a squarefree residual containing `p` adds exactly one to
the `p`-adic exponent. -/
...
/-- The next prime power does not divide: the contact depth is exact. -/
...
/-- The next right contact prime power does not divide. -/
...
/-- Once contact depth is known to be exponent plus one, its weighted excess
is exactly the total modulus-exponent height. -/
