...
Then `n = L₁(n) * L₂(n) * Q₂(n)`, the square `L₂(n)^2` divides `n`,
and

`log Q₁(n) = log L₂(n) + log Q₂(n)`.

Thus every lower bound for the first multiplicity quotient splits into a large
genuine square divisor or a large deeper multiplicity quotient. The
construction uses only gcd and division.
-/
...
theorem both_endpoint_secondLayer_dichotomy_of_height_violation
  ...
  have hfirst :=
    P.both_firstLayerExcessQuotient_scaled_of_height_violation
      hepsilon hviolation
  constructor
  · exact scaled_firstExcess_dichotomy P.c_pos hfirst.1
  · exact scaled_firstExcess_dichotomy
      P.largeEndpoint_pos hfirst.2
