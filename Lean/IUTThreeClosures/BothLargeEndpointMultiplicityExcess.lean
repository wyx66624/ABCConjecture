...
forces a positive logarithmic multiplicity excess not merely on one endpoint,
but separately on both `M` and `c`.  The reason is elementary: each endpoint
radical is bounded by the full conductor, while `M >= c/2`.

This is stronger than the one-endpoint signed-defect localization.  It is the
uniform input needed for simultaneous square-part extraction and the ensuing
canonical generalized Pell equation.

No abc estimate, square-part distribution theorem, or Pell bound is assumed.
-/
...
theorem largeEndpointRadicalLog_le_conductor (P : ABCPoint) :
  ...
  have hsmall := P.singleRadicalLog_nonneg P.endpointMin
  have hc := P.singleRadicalLog_nonneg P.c
...
theorem cRadicalLog_le_conductor (P : ABCPoint) :
  ...
  have hsmall := P.singleRadicalLog_nonneg P.endpointMin
  have hlarge := P.singleRadicalLog_nonneg P.largeEndpoint
  rw [P.conductor_eq_signedLayer_threeRadicalLogs]
  linarith
...
/-- Once the height dominates the fixed constant, both endpoint multiplicity
excesses have the same explicit positive height slope. -/
theorem both_largeEndpoint_multiplicityExcess_positiveSlope_of_height_violation
  ...
  constructor <;> nlinarith [hscaled.1, hscaled.2]
