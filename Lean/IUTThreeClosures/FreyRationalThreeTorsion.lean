...
/-!
# A source-derived rational three-torsion Frey family

For a primitive positive triple `a + b = c`, consider the two Tate-normal-form
curves
...
The affine point `(0,0)` is the distinguished flex of the Tate normal form and
has order three over `Q`.  This module formalizes the source-derived
Weierstrass models, their exact invariants and `j`-maps, and the radical support
of their discriminants.  In particular

`Delta(E_a) = -27*a^8*b*c^3`,
`Delta(E_b) = -27*b^8*a*c^3`,

so no point-dependent bad prime is introduced beyond the original abc
support.  The associated modular parameter `t = 27a/c` satisfies

`j(E_a) = t*(t-24)^3/(t-27)`,

which is a degree-four rational map.  The two orientations also have raw
`j`-denominators `b*c^3` and `a*c^3`; one of them is at least `c^4/2`.

The fixed-cancellation estimate needed to turn this raw quartic corridor into
an exact rational Weil-height corridor is developed in the companion module.
No height-conductor, Szpiro, IUT, Vojta, or abc estimate is assumed here.
-/
