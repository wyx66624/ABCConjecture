...
If an endpoint has logarithmic decomposition `T = K + 4*q`, where `q` is the
logarithm of an extracted fourth root and `K` is the logarithm of the remaining
fourth-power-free coefficient, then the aggregate gain gives an explicit power
saving for `K`.
-/
