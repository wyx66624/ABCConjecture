...
The logarithmic multiplicity excess can be represented by a concrete integer
without first formalizing a canonical square decomposition.  Define
...
for positive `n`.  Combining this with the simultaneous endpoint theorem shows
that every abc violation forces `Q₁(max(a,b))` and `Q₁(c)` both to have a fixed
positive height slope.

No abc conclusion or distribution statement is assumed.
-/
