...
Define `Q₁(n) = n / gcd(n, rad(n))`.  The exact gcd identity gives
`n <= rad(n) * Q₁(n)`, so every abc violation forces both `Q₁(max(a,b))`
and `Q₁(c)` to have a fixed positive height slope.
-/
