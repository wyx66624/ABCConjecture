...
/-!
# Version 9 torsion cancellation and incidence cores

This module formalizes two unconditional elementary pieces of the version 9
research programme.

* The canonical and noncanonical Tate-line coefficients cancel on one complete
  projective orbit, and the generic `1 / (ell + 1)` selector meets the
  noncanonical baseline at exactly zero.
...
No elliptic-curve source, amplifier, IUT theorem, or `ABCConjecture` conclusion
is assumed.
-/
...
/-- Coefficient of the canonical cyclic `ell`-line. -/
def tateCanonicalLineCoefficient (ell : ℝ) : ℝ :=
  (ell - 1) / 12

/-- Coefficient of each noncanonical cyclic `ell`-line. -/
def tateNoncanonicalLineCoefficient (ell : ℝ) : ℝ :=
  -((ell - 1) / (12 * ell))
...
/-- The generic `1 / (ell + 1)` selector meets the noncanonical baseline at
exactly zero. -/
...
/-- Complete-orbit cancellation after multiplication by an arbitrary total
weight. -/
...
/-- View an incidence as an input together with an output in its fiber. -/
def equivLeft :
...
/-- View an incidence as an output together with an input in its fiber. -/
def equivRight :
  ...
  left_inv p := by
    ...
    rfl
  right_inv p := by
    rcases p with ⟨b, ⟨a, hab⟩⟩
    rfl

variable [Fintype α] [Fintype β]

/-- Count incidences by input fibers. -/
...
/-- Count incidences by output fibers. -/
