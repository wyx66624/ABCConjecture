...
/-- In particular the shifted parameter is nonzero. -/
...
theorem shiftedJ_not_integer (P : ABCPoint) :
  ...
  have hone : P.shiftedJ.den = 1 := by simpa using hden
  exact P.shiftedJ_den_ne_one hone
...
theorem height_add_log_two_le_shiftedJHeight (P : ABCPoint) :
  ...
  have hc : (0 : ℝ) < P.c := by
    exact_mod_cast P.c_pos
  have hle : (2 : ℝ) * P.c ≤ ((2 * P.c + P.a : ℕ) : ℝ) := by
