...
/-- The `n`-th translated canonical slice. -/
def sheet (x : X) (n : ℤ) : Set X :=
  A.act n '' A.slice x

/-- Every translated slice is open. -/
theorem isOpen_sheet (x : X) (n : ℤ) :
    IsOpen (A.sheet x n) :=
  (A.translateHomeomorph n).isOpenMap _ (A.isOpen_slice x)

/-- Applying an inverse translate compares two translated points. -/
...
/-- Distinct integer translates of a canonical slice are disjoint. -/
...
/-- Every sheet has the same image in the orbit quotient. -/
...
/-- The quotient map is injective on every translated sheet. -/
theorem quotientMap_injOn_sheet
  ...
  have hq₀ : A.quotientMap y₀ = A.quotientMap z₀ := by
    calc
      A.quotientMap y₀ = A.quotientMap (A.act n y₀) := by
        apply Quotient.sound
        exact ⟨n, rfl⟩
      _ = A.quotientMap (A.act n z₀) := hq
      _ = A.quotientMap z₀ := by
        apply Quotient.sound
        exact ⟨n, rfl⟩
  have hyz : y₀ = z₀ :=
...
/-- The quotient map is surjective from every sheet onto the canonical quotient
neighborhood. -/
...
/-- The inverse image of the canonical quotient neighborhood is the union of
all translated sheets. -/
...
/-- Package the complete local covering data over one canonical quotient
neighborhood. -/
...
/-- Every point supplies a canonical local covering chart. -/
...
/-- Every quotient point is contained in a neighborhood admitting the explicit
integer-indexed covering decomposition. -/
