...
In the non-short-gap branch, write `Tm, TM, Tc` for the three logarithmic
endpoint sizes, `rm, rM, rc` for their radical weights and `qm, qM, qc` for
the logarithmic sizes of their canonical fourth roots.  The elementary
fourth-power decomposition gives

`Ti <= 3 * ri + 4 * qi`.

Combining all three inequalities with an abc violation gives the exact scalar
budget
...
Consequently one endpoint carries at least one third of this aggregate gain.
No abc estimate, distribution theorem, or arithmetic-existence statement is
...
theorem totalWeight_le_three_mul_radical_add_four_mul_fourthRootWeight
  ...
  norm_num at hdecomp hres ⊢
  rw [hdecomp]
  nlinarith

/-- Aggregate scalar fourth-root gain.  `Lm`, `LM`, and `Lc` are the three
explicit logarithmic losses in the endpoint lower bounds. -/
theorem aggregate_fourthRoot_gain
  ...
  have hscale_nonneg : 0 ≤ 2 * (1 + epsilon) := by
    nlinarith
  have hM_scaled :=
  ...
  have hsum_upper :
      Tm + TM + Tc ≤
        3 * (rm + rM + rc) + 4 * (qm + qM + qc) := by
    nlinarith
  have hsum_scaled :=
    mul_le_mul_of_nonneg_left hsum_upper hscale_nonneg
  have hviolation_scaled :=
    mul_lt_mul_of_pos_left hviolation (by norm_num : (0 : ℝ) < 6)
  nlinarith

/-- If the aggregate fourth-root budget is positive, one of the three
endpoints carries at least one third of it.  The denominator-free formulation
avoids any hidden positivity or division convention. -/
