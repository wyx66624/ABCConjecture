...
The original radical splits into the radical of the root triple
`d+x=y` and the additional radical of `s` (up to the harmless prime two).
This file records the exact scalar dichotomy.

For any `epsilon>0`, put

`alpha(epsilon) = 2 / ((1+epsilon)*(2+epsilon))`.

If the `s`-radical is at least `alpha*h` (up to a fixed loss), the original
ABC violation transfers to the root triple with exponent `epsilon/2`.
If it is smaller, the canonical square root extracted from `s` has a fixed
positive height slope.  Thus the split-square branch either descends to a
smaller ABC violation or creates a new height-scale square divisor.

No ABC estimate, radical factorization theorem, or arithmetic-existence input
is assumed; the declarations below are exact real inequalities.
-/
...
theorem rootConductor_transfer
  ...
  have hbase :
    ...
    nlinarith
  have hratio : 0 < (1 + eta) / (1 + epsilon) :=
    div_pos hetapos hepspos
  have hscaled := mul_lt_mul_of_pos_left hbase hratio
  have hratioCoefficient :
      ((1 + eta) / (1 + epsilon)) *
          (2 - (1 + epsilon) * alpha) ≤ 1 := by
    apply (div_le_iff₀ hepspos).2
    nlinarith
  have hright :=
    mul_le_mul_of_nonneg_right hratioCoefficient hH
  have hleftIdentity :
      ((1 + eta) / (1 + epsilon)) *
          ((1 + epsilon) * Rroot + C - (1 + epsilon) * K) =
        (1 + eta) * Rroot +
