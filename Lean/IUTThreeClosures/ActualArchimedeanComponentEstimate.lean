...
* an honest finite-positive measure model of the public theta-hull component;
* a normalized unit region of logarithmic volume zero;
* the genuine radial `π`-scaling law;
...
`componentVol(thetaHullComponent) ≤ card(S) * log π`.

Thus `actualArchimedeanComponentEstimate` is reduced to geometric
identification and containment data, rather than being accepted as a free
real-valued bound.
-/
...
structure ActualArchimedeanComponentMetricData
  ...
  /-- The actual radial Haar measure on each complex direct summand. -/
  measure :
  ...
  thetaRegion :
    ∀ (i : Fin R.container.proc.length)
      ...
      FinitePositiveRegion
        ((R.container.packet i .infinite).Summand c) (measure i c)
  ...
  unitRegion :
    ∀ (i : Fin R.container.proc.length)
      ...
      FinitePositiveRegion
        ((R.container.packet i .infinite).Summand c) (measure i c)
  unit_logVolume : ∀ i c, (unitRegion i c).logVolume = 0
  /-- The set-level radial scaling by `π`. -/
  piTransform :
    ∀ (i : Fin R.container.proc.length)
      (c : R.container.Components i .infinite),
      Set ((R.container.packet i .infinite).Summand c) →
        Set ((R.container.packet i .infinite).Summand c)
  /-- Genuine radial scaling: one application adds `log π`. -/
  piScaling : ∀ i c,
    FinitePositiveRegion.ScalingLaw
      (μ := measure i c) (piTransform i c) (Real.log Real.pi)
  /-- IUT IV Proposition 1.5: the actual component is contained in the
  `card(S)`-fold radial `π`-container. -/
