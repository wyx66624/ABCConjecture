...
leaves an even denominator. Thus the negative two-adic contribution cannot be
erased by any further ordinary gcd cancellation.
-/
...
theorem freyTwoResidualDenominator_even
  ...
  have hexponent : 2 * n - 8 = (2 * n - 9) + 1 := by omega
  refine ⟨2 ^ (2 * n - 9) * u ^ 2, ?_⟩
  rw [freyTwoResidualDenominator, hexponent, pow_succ]
  ring
...
theorem cube_odd {Q : ℕ} (hQ : Q.Odd) :
    (Q ^ 3).Odd := by
  simpa [pow_succ] using hQ.mul (hQ.mul hQ)
...
/-- For `n>0`, the first unit-leg quadratic factor
`1 + 2^n + (2^n)^2` is odd. -/
theorem unitLegPlusQ_odd
    {n : ℕ} (hn : 0 < n) :
    (1 + 2 ^ n + (2 ^ n) ^ 2).Odd := by
  rcases Nat.exists_eq_succ_of_ne_zero (Nat.ne_of_gt hn) with ⟨k, rfl⟩
  have hpow : Even (2 ^ (k + 1)) := by
    refine ⟨2 ^ k, ?_⟩
    rw [pow_succ]
    ring
