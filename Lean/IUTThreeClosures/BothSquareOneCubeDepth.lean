...
adjacent endpoints.  Signed exponent-two localization additionally forces one
of those endpoints to have a large second-layer quotient.  Refining that
quotient gives a genuine cube divisor or a still deeper remainder.

All layers are defined by gcd and natural-number division.
-/
...
theorem le_radical_sq_mul_secondLayerExcessQuotient (n : ℕ) :
  ...
  have h12 :
      firstSupportLayer n * secondSupportLayer n ≤
        abcRadical n * abcRadical n :=
    Nat.mul_le_mul (firstSupportLayer_le_radical n)
      (secondSupportLayer_le_radical n)
  have hmul :=
    Nat.mul_le_mul_right (secondLayerExcessQuotient n) h12
  rw [firstSupport_mul_secondSupport_mul_secondExcess_eq n] at hmul
