...
No PNT theorem is postulated as a structure field: the main theorem takes the
standard convergence statement as an ordinary theorem argument, so it can be
instantiated directly by the Mathlib PNT theorem once its namespace-level
form is imported.
-/
