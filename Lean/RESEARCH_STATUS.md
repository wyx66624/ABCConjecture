...
Positive construction and full-premise counterexample search continue in
parallel.  Difficulty and finite no-hit evidence retire no route; a verified
counterexample retires only the exact statement whose complete premises it
satisfies.

- **Refined IUT-facing Haar route:** active.  Primitive local factors give the
  exact `e*f` coefficient, total-degree product-Haar normalization, relative
  weights, compact-open orbit control, and the conditional pointed sandwich.
  Factor-count, copied-weight, envelope-only, and unpointed variants are
  refuted at their exact scopes.  Concrete IPL/SHE/APT, Ind3 positivity, and
  same-object pointed transport remain open.
- **Mersenne Farey-entropy route:** active.  The exact finite denominator
  split turns linear endpoint energy into a polynomial swarm of distinct
  depth-three exact-order rows.  Its critical global counting exponent is an
  explicit paper-level open hypothesis.  The common-index model refutes only
  the inference with primality, exact order, and depth deleted.
- **Affine ownership route:** active.  Maximal-top support uniqueness,
  ownership Cauchy aggregation, strict non-arm reduction, and sharp numerical
  boundaries are proved.  The analytic count now correctly assumes `C=B+1`.
  A canonical catalogue-sparsity or supersaturation estimate remains open.
- **Polynomial Pell--Hensel route:** active.  All-index norm and derivative
  identities, arbitrary-polynomial one-digit Hensel lifting, and a bundled
  full-premise `T=282` moving witness are proved.  The witness changes the
  Pell coefficient and does not satisfy the fixed `D=2` squarefull packet;
  that unbounded exclusion remains open.
- **Valuation-contact route:** active after exact local retirements.  The
  Bloch five-term divisor surface, integer exponent-gcd/base split, concrete
  finite-support norm, conditional exact-boundary filling inequality,
  quadratic Veronese peeling, and valuation-layer expansion are proved.
  Primitive Pythagorean squares refute both single-cell gates and three fixed
  one-move policies, but not arbitrary multi-cell fillings.  The inductive
  relation generated by permitted five-term moves and its boundary theorem
  remain unformalized; Gate VF also retains two genuine uniform cost estimates.

The detailed mathematical ledger is
`../research/ABC_MULTI_ROUTE_REFINED_HAAR_FAREY_OWNERSHIP_HENSEL_CONTACT_2026_09_02.md`;
the original five-route full-premise review is
`../research/ABC_FIVE_ROUTE_ADVERSARIAL_REVIEW_2026_09_02.md`; the supplemental
Pell, Steinberg finite-chain, and quadratic reports carry their own quantifier
audits.  The eight Lean modules contain 297 theorem declarations, 113
definitions, five abbreviations, four structures, and two named instances,
for 421 counted declarations.  The generated cross-audit checks all 421
declarations one for one; its dependency union is exactly `propext`,
`Classical.choice`, and `Quot.sound`.  All eight direct warning-as-error
compilations and the audit pass, the aggregate target completes 9,255 jobs,
and all specified deterministic evidence replays pass.  The immutable record
is `verification/2026_09_02_refined_haar_farey_ownership_hensel_contact/`.
None of these modules constructs `ABCConjecture` or its negation.
