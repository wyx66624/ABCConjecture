#!/usr/bin/env python3
"""Reproduce the determinant/totient/refined-signature Lean checkpoint.
...
the three checkpoint modules, deterministically generates one independent
``#print axioms`` command per proof declaration, compiles the three modules and
the generated audit, and rebuilds the aggregate target.

The generated audit recompiles an exact amalgamation of the three source files
...
MODULES = (
    "AffineDeterminantLayerEntropy20260901",
    "MersenneTotientDivisorConcentration20260901",
    "IUTRefinedFactorZeroAwareSignature20260901",
)
MODULE_PATHS = tuple(
...
def validate(record: bool) -> None:
    ...
    audit_run, audit_output = run_logged(
        output_root,
        "axiom-audit",
        [lake, "env", "lean", relative_to_repo(audit_path)[len("Lean/") :]],
        LEAN_ROOT,
    )
    ...
    inventory_document = {
        "schema": "abc-determinant-totient-refined-signature-inventory-v1",
        "modules": inventories,
        "totals": {
            **totals,
    ...
    result = {
        "schema": "abc-determinant-totient-refined-signature-validation-v1",
        "status": "PASS",
...
def verify_recorded_structure() -> dict[str, Any]:
    ...
    if result.get("schema") != "abc-determinant-totient-refined-signature-validation-v1":
