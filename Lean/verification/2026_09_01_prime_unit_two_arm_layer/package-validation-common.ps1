...
function Test-RecordedValidationPackage {
  ...
  $expectedSchema = 'abc-prime-unit-two-arm-layer-validation-v1'
  $expectedModules = @(
  ...
  if ($crossRows.Count -ne 1) { throw 'cross-reference manifest inventory is not exact' }
  $cross = $crossRows[0]
  $expectedCrossManifest = 'research/computation/2026_09_01_mersenne_prime_layer_radical/DELIVERY_MANIFEST.json'
  if ([string](Get-RequiredProperty $cross 'manifest' 'cross-reference manifest') -cne $expectedCrossManifest) {
    throw 'unexpected cross-reference manifest path'
  }
  $crossManifestSha = [string](Get-RequiredProperty $cross 'manifestSha256' 'cross-reference manifest')
  Assert-LowerSha256 $crossManifestSha 'cross-reference manifest hash'
  if (-not $inputHashByPath.ContainsKey($expectedCrossManifest) -or
      $inputHashByPath[$expectedCrossManifest] -cne $crossManifestSha) {
    throw 'cross-reference manifest hash does not match the frozen input manifest'
  }
  $expectedReferenceLabels = @('report', 'lean_module', 'source_ledger')
  ...
  $referenceRows = @(Get-RequiredProperty $cross 'references' 'cross-reference manifest')
  $actualReferenceLabels = @($referenceRows | ForEach-Object {
      [string](Get-RequiredProperty $_ 'label' 'cross-reference row')
    })
  Assert-ExactStringSequence $actualReferenceLabels $expectedReferenceLabels 'cross-reference labels'
  for ($index = 0; $index -lt $referenceRows.Count; $index++) {
    $row = $referenceRows[$index]
    $path = [string](Get-RequiredProperty $row 'path' 'cross-reference row')
    if ($path -cne $expectedReferencePaths[$index]) {
      throw "cross-reference path mismatch for $($expectedReferenceLabels[$index])"
    }
    $referenceSha = [string](Get-RequiredProperty $row 'sha256' "cross-reference '$path'")
    Assert-LowerSha256 $referenceSha "cross-reference '$path' sha256"
    if (-not $inputHashByPath.ContainsKey($path) -or
        $inputHashByPath[$path] -cne $referenceSha) {
      throw "cross-reference '$path' does not match the frozen input manifest"
    }
  }
  ...
  if ($aggregateJobs -le 0) { throw 'aggregate build job count is not positive' }
