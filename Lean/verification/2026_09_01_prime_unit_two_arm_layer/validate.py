#!/usr/bin/env python3
"""Reproduce the prime/unit, two-arm, and Mersenne continuation.
...
MODULES = (
    "CarellaGlobalOmegaHypothesis20260901",
    "IUTPrimeUnitLabelVectorBridge20260901",
    "AffineTwoArmCRTPacket20260901",
    "MersennePrimeLayerRadical20260901",
    "MersenneOrderBlockDecomposition20260901",
    "MersenneOrderBlockAsymptotic20260901",
    "MersenneCanonicalBlockWitness20260901",
    "MersenneWieferichTailReduction20260901",
)
...
NON_LEAN_INPUT_PATHS = (
        ...
        "paper/carella_global_omega_2026.tex",
        "paper/iut_prime_unit_label_vector_2026.tex",
        "paper/affine_two_arm_crt_2026.tex",
        "paper/mersenne_prime_layer_radical_2026.tex",
        "research/computation/2026_09_01_affine_two_arm_crt_packet/SHA256SUMS",
        "research/computation/2026_09_01_mersenne_prime_layer_radical/SHA256SUMS",
        "research/computation/2026_09_01_mersenne_prime_layer_radical/DELIVERY_MANIFEST.json",
        "research/sources/affine_two_arm_crt_2026_09_01/SHA256SUMS",
        "research/sources/affine_two_arm_crt_2026_09_01/SOURCE_MANIFEST.md",
        "research/sources/iut_prime_unit_label_vector_bridge_2026_09_01/SHA256SUMS",
        "research/sources/iut_prime_unit_label_vector_bridge_2026_09_01/source-metadata.json",
        "research/sources/mersenne_prime_layer_radical_2026_09_01/SHA256SUMS",
        "research/sources/mersenne_prime_layer_radical_2026_09_01/source-metadata.json",
)
...
def verify_mersenne_delivery_references(
    inventories: list[dict[str, object]],
) -> dict[str, object]:
    """Check cross-directory hashes pinned by the Mersenne delivery manifest."""
    relative = (
        "research/computation/2026_09_01_mersenne_prime_layer_radical/"
        "DELIVERY_MANIFEST.json"
    )
    manifest = repo_path(relative)
    data = json.loads(manifest.read_text(encoding="utf-8"))
    if data.get("schema") != "mersenne-prime-layer-delivery-v1":
        raise RuntimeError("unexpected Mersenne delivery-manifest schema")
    expected_paths = {
        "report": "research/ABC_MERSENNE_PRIME_LAYER_RADICAL_2026_09_01.md",
        "lean_module": "Lean/IUTThreeClosures/MersennePrimeLayerRadical20260901.lean",
        "source_ledger": (
            "research/sources/mersenne_prime_layer_radical_2026_09_01/"
            "SHA256SUMS"
        ),
    ...
    for label, expected_path in expected_paths.items():
        ...
        if not isinstance(row, dict):
            raise RuntimeError(f"Mersenne delivery manifest is missing {label}")
        recorded_path = row.get("path")
        recorded_hash = row.get("sha256")
        if recorded_path != expected_path:
            raise RuntimeError(
                f"Mersenne delivery {label} path changed: {recorded_path!r}"
            )
        if not isinstance(recorded_hash, str) or not re.fullmatch(
            r"[0-9a-f]{64}", recorded_hash
        ):
            raise RuntimeError(f"Mersenne delivery {label} has malformed SHA-256")
        actual_hash = sha256(repo_path(recorded_path))
        if recorded_hash != actual_hash:
            raise RuntimeError(
                f"Mersenne delivery {label} hash is stale: "
                f"recorded={recorded_hash}, actual={actual_hash}"
            )
        references.append(
            {"label": label, "path": recorded_path, "sha256": actual_hash}
        )

    prime_inventory = next(
        row
        for row in inventories
        if row["module"] == "MersennePrimeLayerRadical20260901"
    )
    lean_row = data["lean_module"]
    proof_count = len(prime_inventory["proofDeclarations"])
    if lean_row.get("theorem_declarations") != proof_count:
        raise RuntimeError("Mersenne delivery theorem count is stale")
    if lean_row.get("print_axioms_commands") != prime_inventory["printAxiomsCommands"]:
        raise RuntimeError("Mersenne delivery #print axioms count is stale")
    if lean_row.get("compile_exit_code") != 0:
        raise RuntimeError("Mersenne delivery compile exit code is not zero")
    if lean_row.get("sorryAx") is not False or lean_row.get("native_decide_axiom") is not False:
        raise RuntimeError("Mersenne delivery records a forbidden proof dependency")
    return {
        "manifest": relative,
        "manifestSha256": sha256(manifest),
...
def main() -> None:
    ...
    cross_reference_result = verify_mersenne_delivery_references(inventories)

    python = sys.executable
    affine_dir = repo_path("research/computation/2026_09_01_affine_two_arm_crt_packet")
    affine_run = run_logged(
        "affine-two-arm-replay",
        [python, "-B", "verify_two_arm_packet.py"],
        affine_dir,
        require="captured_output_match=true",
    )
    affine_run.pop("output")

    iut_dir = repo_path("research/sources/iut_prime_unit_label_vector_bridge_2026_09_01")
    iut_run = run_logged(
        "iut-source-ledger-replay",
        [python, "-B", "verify_source_metadata.py"],
        iut_dir,
        require="PASS 4 referenced primary-source files",
    )
    iut_run.pop("output")

    mersenne_dir = repo_path("research/computation/2026_09_01_mersenne_prime_layer_radical")
    with tempfile.TemporaryDirectory(prefix="abc-mersenne-replay-") as temporary:
        temp = Path(temporary)
        scan_output = temp / "prime_layers_3_61.json"
        verification_output = temp / "verification.json"
        scan_run = run_logged(
            "mersenne-prime-layer-scan",
            [
                python,
                "-B",
                str(mersenne_dir / "scan_prime_layers.py"),
                "--max-index",
                "61",
                "--output",
                str(scan_output),
        ...
        scan_run.pop("output")
        compare_bytes(
            scan_output, mersenne_dir / "prime_layers_3_61.json", "Mersenne scan"
        )
        verify_run = run_logged(
            "mersenne-prime-layer-verify",
            [
                python,
                "-B",
                str(mersenne_dir / "verify_prime_layers.py"),
                str(scan_output),
                "--output",
    ...
    result = {
        "schema": "abc-prime-unit-two-arm-layer-validation-v1",
        "status": "PASS",
        ...
        "crossReferences": [cross_reference_result],
        "replays": [affine_run, iut_run, scan_run, verify_run],
        "scope": {
