...
\begin{document}
\begin{abstract}
We prove an effective finiteness statement for the positive integer family
$b+2=3r_0^2$, $b+3=s^2$, and $b^2+3b+1=T_p(X)$, where $p\ge3$ and
$X>1$. Matveev's normalized coefficient bound gives the absolute estimate
$p<2^{59}$; an effective integral-point theorem then bounds every parameter.
This family is not known to contain all possible $abc$ exceptions.
We also examine other uniformity requirements in approaches to $abc$.
An elementary encoding bounds the number of integers $n\le N$
...
its endpoints separately. Quantitative counts limit two specified
amplification constructions. For local tensor constructions, we prove an
exact determinant certificate for the Haar measure of a generated lattice
and a full-lattice containment inside the specified holomorphic hull.
The tensor order, its normalization, and ordinary convex closure must be
distinguished. Recent logarithmic-form methods give further endpoint
estimates, while explicit Frey invariants delimit several direct transfers.
Arithmetic, finite counting, local Haar, and scalar analytic components are
checked in Lean; external theorems and unproved global interfaces are
...
dependencies. The eleven principal companion modules are listed below.
...
outside that conclusion. No unconditional closed term
