#!/usr/bin/env python3
"""Reproduce the source/multiplier/catalogue/Lucas checkpoint.
...
MODULES = (
    "AffineCatalogueWeightOverlap20260901",
    "IUTRationalDegreeOneSourceRealization20260901",
    "MersenneBalancedMultiplierDepthLocalization20260901",
    "PellLucasAllOrderStaircase20260901",
)
MODULE_PATHS = tuple(
...
EXTRA_INPUT_PATHS = (
    ...
    "paper/affine_catalogue_weight_overlap_2026.tex",
    "paper/iut_rational_degree_one_source_realization_2026.tex",
    "paper/mersenne_balanced_multiplier_depth_localization_2026.tex",
    "paper/mersenne_balanced_multiplier_depth_localization_2026_bibitems.tex",
    "paper/pell_lucas_all_order_staircase_2026.tex",
    "paper/pell_lucas_all_order_staircase_2026_bibitems.tex",
    "output/pdf/ChatGPT_ABC_Uniformity_2026.pdf",
)
EXTRA_INPUT_DIRECTORIES = (
    "research/computation/2026_09_01_pell_lucas_all_order",
    "research/sources/mersenne_balanced_multiplier_depth_2026_09_01",
    "research/sources/pell_fourth_order_lucas_2026_09_01",
    "research/sources/iut_rational_degree_one_source_realization_2026_09_01",
    "output/pdf/ChatGPT_ABC_Source_Multiplier_Catalogue_Lucas_2026_09_01_QA",
)
EXPECTED_AGGREGATE_JOBS = 9229
LUCAS_REPLAY_RELATIVE = "research/computation/2026_09_01_pell_lucas_all_order"
LUCAS_REPLAY_PACKET = "lucas_all_order_packet.json"
LUCAS_REPLAY_OUTPUT = "lucas-all-order-independent-verification.json"
EXPECTED_LUCAS_TARGET_RESIDUES = (0, 1, 2, 3, 4)
EXPECTED_LUCAS_VERIFIED = {
...
EXPECTED_COUNTED_DECLARATIONS = 86
EXPECTED_PROOF_DECLARATIONS = 67
...
def verify_lucas_target_residues(packet: Any) -> list[int]:
    if not isinstance(packet, dict):
        raise RuntimeError("Pell--Lucas packet is not a JSON object")
    rows = packet.get("all_residues_mod_5")
    if not isinstance(rows, list):
        raise RuntimeError("Pell--Lucas packet has no local residue-row list")
    residues: list[int] = []
    for index, row in enumerate(rows):
        if not isinstance(row, dict):
            raise RuntimeError(f"Pell--Lucas local row {index} is not an object")
        residue = row.get("target_residue")
        if type(residue) is not int:
            raise RuntimeError(
                f"Pell--Lucas local row {index} has a noninteger target residue"
            )
        residues.append(residue)
    expected = list(EXPECTED_LUCAS_TARGET_RESIDUES)
    if len(rows) != len(expected) or sorted(residues) != expected:
        raise RuntimeError(
            f"Pell--Lucas target residue set changed: {sorted(residues)} != {expected}"
        )
    return expected


def load_lucas_target_residues(directory: Path) -> list[int]:
    packet_path = directory / LUCAS_REPLAY_PACKET
    if not packet_path.is_file():
        raise RuntimeError(f"missing Pell--Lucas replay packet: {packet_path}")
    packet = json.loads(packet_path.read_text(encoding="utf-8"))
    return verify_lucas_target_residues(packet)
...
def validate(record: bool) -> None:
    ...
    pell_directory = canonical_repo_path(LUCAS_REPLAY_RELATIVE)
    pell_output_path = output_root / LUCAS_REPLAY_OUTPUT
    pell_run, pell_output = run_logged(
        output_root,
        "pell-lucas-replay",
        [sys.executable, "-B", "verify_lucas_all_order_packet.py"],
        pell_directory,
    )
    frozen_pell_output = pell_directory / "lucas_all_order_verification.json"
    shutil.copyfile(frozen_pell_output, pell_output_path)
    pell_document = json.loads(pell_output_path.read_text(encoding="utf-8"))
    pell_target_residues = load_lucas_target_residues(pell_directory)
    if pell_document != EXPECTED_LUCAS_VERIFIED:
        raise RuntimeError("Pell--Lucas replay document changed")
    if pell_document.get("local_rows") != len(pell_target_residues):
        raise RuntimeError("Pell--Lucas replay row count disagrees with its packet")
    if "PASS" not in pell_output:
        raise RuntimeError("Pell--Lucas replay stdout lacks its PASS marker")
    pell_run["output"] = relative_to_repo(pell_output_path)
    pell_run["outputSha256"] = sha256(pell_output_path)
    pell_run["verified"] = pell_document
    pell_run["targetResidues"] = pell_target_residues
    ...
    inventory_document = {
        "schema": "abc-source-multiplier-catalogue-lucas-inventory-v1",
        "modules": inventories,
        "totals": {
            **totals,
    ...
    result = {
        "schema": "abc-source-multiplier-catalogue-lucas-validation-v1",
        "status": "PASS",
    ...
    summary = (
        ...
        f"lucas_local_rows={EXPECTED_LUCAS_VERIFIED['local_rows']}\n"
        f"lucas_target_residues={','.join(map(str, EXPECTED_LUCAS_TARGET_RESIDUES))}\n"
        f"lucas_sample_indices={','.join(map(str, EXPECTED_LUCAS_VERIFIED['sample_indices']))}\n"
...
def verify_recorded_structure() -> dict[str, Any]:
    ...
    if result.get("schema") != "abc-source-multiplier-catalogue-lucas-validation-v1":
    ...
    if inventory_document.get("schema") != (
        "abc-source-multiplier-catalogue-lucas-inventory-v1"
    ):
    ...
    pell_output = PACKAGE_ROOT / LUCAS_REPLAY_OUTPUT
    ...
    if pell_run.get("verified") != EXPECTED_LUCAS_VERIFIED:
        raise RuntimeError("recorded Pell replay counts changed")
    if pell_run.get("targetResidues") != list(EXPECTED_LUCAS_TARGET_RESIDUES):
        raise RuntimeError("recorded Pell target residue set changed")
    return result
