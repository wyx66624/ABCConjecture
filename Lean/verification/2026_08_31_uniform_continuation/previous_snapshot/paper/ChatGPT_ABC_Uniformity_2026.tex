...
\begin{document}
\begin{abstract}
We prove several estimates and structural statements relevant to the
uniformity problem in $abc$. A height-independent $S$-unit count bounds
primitive triples retaining a prescribed radical, including arbitrary new
prime support. Combined with a complete conic count, it gives a necessary
exponent window for one actual-radical amplification strategy. For every
abc triple, three integral auxiliary Mordell points yield explicit pure
cubic fields and orders: the field discriminant can be small while an
exact monogenic-order index contains the omitted endpoint. Recent
Thue estimates then give a relative height bound. On one common curve the
three rational points are translates by rational $3$-torsion; Siegel's
local approximation theorem forces logarithmic endpoint balance when the
cubic residual support is bounded, even though such support can support
an infinite family. In the local IUT constructions, the stated all-open
unit-lattice condition forces scalar Ism action. This removes Ind2 from
a fixed maximal-order module hull, while a cyclotomic trace invariant
distinguishes specified native root-power inputs under Galois/Kummer
transport. Their pure-orbit separation does not settle the subsequent
hull or Ind3 comparison. Explicit automorphisms of the full local Galois
group attain simultaneous minimum layers for all the square labels in a
tame family. A trace-dual argument computes the exact hull of a
maximal-order ideal formed before transport; the larger whole-product
source has a different hull. Concrete Frey curves realize the local
fields. A uniform power-free CRT count gives related curves at
unbounded height in one fixed compactly bounded domain; every member
admits the original initial theta data by a direct construction.
We also prove effective finiteness of a specified
Pell--Chebyshev family, with an absolute index bound $p<2^{59}$, and give
prime-support and endpoint tests for other proposed arguments. Selected
arithmetic, lattice, and algebraic components are checked in Lean; external
theorems and the remaining global interfaces are kept separate.
Neither a proof nor a disproof of the standard abc conjecture is claimed.
\end{abstract}
\maketitle
...
theorem at unbounded height. No priority over existing literature is
...
been obtained. The remaining task is to identify the complete global
admissible family, its weights and markings, and the Ind3 and
cross-Frobenius comparison for the same measured set. Constructing
initial data or eliminating Ind2 from a fixed $B$-module hull does not
supply that comparison. No unconditional closed term
