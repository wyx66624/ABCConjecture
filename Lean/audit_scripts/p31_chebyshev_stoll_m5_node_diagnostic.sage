...
# The 4000-bit scout certified the complete m=3 shell but lost the required
# identity margin at the first m=4 node.  The frozen run doubles precision;
# every constructed identity must retain valuation strictly above 2000.
prec = ZZ(os.environ.get('P31_DIAGNOSTIC_PRECISION','10000'))
required_identity_valuation = 0
print('P31_M5_DIAGNOSTIC_PRECISION',prec,flush=True)
Qh = Qp(2, prec)
Rh.<X> = Qh[]
...
def solve_half(ap, bp, a0=None, b0=None):
    ...
    global diagnostic_half_counter
    diagnostic_half_counter += 1
    print('P31_M5_HALF_LAYER',diagnostic_half_counter,'D',d,
          'C1',c1,'C2',c2,'C3',c3,'MIN',certificate_valuation,
          'PRECISION',prec,'HEADROOM',prec-certificate_valuation,flush=True)
...
result = shell_node(5,1)
print('P31_M5_UNIT1_RESULT',result,'HALF_LAYERS',diagnostic_half_counter,flush=True)
print('P31_M5_SINGLE_NODE_DIAGNOSTIC_PASS')
