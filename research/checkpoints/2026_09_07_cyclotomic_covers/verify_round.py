#!/usr/bin/env python3
"""Fresh, scoped Std-only compilation of actual Lucas and quartic support arithmetic."""
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='cyclotomic-cover-fresh-', dir=scratch))
    ...
    (build / 'lakefile.toml').write_text(
        'name = "ABCCyclotomicCoverAudit"\nversion = "0.1.0"\n'
        'defaultTargets = ["CyclotomicCoverAudit"]\n[leanOptions]\nwarningAsError = true\n'
        '[[lean_lib]]\nname = "CyclotomicCoverAudit"\nroots = [' + roots + ']\n')
    inventory = {}
    for name, checkpoint in MODULES.items():
        ...
        inventory[name] = {
            ...
            'new': name in {'GeneralLucasBoundary','QuarticSupportArithmetic'},
        }
    ...
    result = {
        ...
        'not_claimed': ['ABC proof or disproof', 'full repository build',
                        ...
                        'complete fifth-round formalization', 'Hensel or Faltings theorem', 'finite Kummer descent', 'external peer review'],
    }
