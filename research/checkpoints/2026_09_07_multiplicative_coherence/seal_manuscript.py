...
BUILD = ROOT / 'tmp/abc_20260907/round12pdf'
PREVIOUS_SHA = 'a218aa002ee3ffdbcef99d13b447a69ddb8b213b374e2bc37c1028a91b748021'
...
def main():
    ...
    for relative, digest in build_sources.items():
        assert sha(ROOT / relative) == digest, relative
    prior = json.loads((ROOT / 'research/checkpoints/2026_09_07_integral_lifting/verification/manuscript_validation.json').read_text())
    ...
    assert unchanged_children == review['previous_unchanged_child_sources'] == 175
    ...
    assert formal['new_declarations'] == 21 and formal['dependency_declarations'] == 29
    assert len(formal['modules']) == 3
    ...
    assert finite['status'].startswith('PASS:') and len(finite['records']) == 3
