#!/usr/bin/env python3
"""Fresh bounded adaptive-owner arithmetic and complete axiom queries.
...
SOURCES = {
    ...
    'AdaptiveOwnerArithmetic': HERE / 'Lean/AdaptiveOwnerArithmetic.lean',
}
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='adaptive-owner-fresh-', dir=scratch))
    modules = {}
    for name, source in SOURCES.items():
        ...
        modules[name] = dict(source=source.relative_to(ROOT).as_posix(),
                            ...
                            new=(name == 'AdaptiveOwnerArithmetic'),
    ...
    record = dict(status='RUNNING: fresh source compilation not yet successful',
                  ...
                  modules=modules)
    result_path = out / 'owner_validation.json'
    ...
    for name, module in modules.items():
        ...
        if proc.returncode or 'sorryAx' in current or re.search(r'\berror:', current):
            (out / 'owner-lean.log').write_bytes(('\n'.join(logs)).encode())
    ...
    log = '\n'.join(logs)
    (out / 'owner-lean.log').write_bytes(log.encode())
    union = set()
    for query in [q for module in modules.values() for q in module['queries']]:
        ...
        union |= axioms
    record.update(status='PASS: fresh adaptive-owner finite arithmetic, complete axiom audit',
