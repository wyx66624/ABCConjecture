#!/usr/bin/env python3
"""Fresh bounded uniform-moment arithmetic and complete axiom queries.
...
SOURCES = {
    'UniformMomentArithmetic': HERE / 'Lean/UniformMomentArithmetic.lean',
}
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='uniform-moment-fresh-', dir=scratch))
    modules = {}
    ...
    record = dict(status='RUNNING: fresh source compilation not yet successful',
                  ...
                  modules=modules)
    result_path = out / 'moment_validation.json'
    ...
    for name, module in modules.items():
        ...
        if proc.returncode or 'sorryAx' in current or re.search(r'\berror:', current):
            (out / 'moment-lean.log').write_bytes(('\n'.join(logs)).encode())
    ...
    log = '\n'.join(logs)
    (out / 'moment-lean.log').write_bytes(log.encode())
    union = set()
    for query in [q for module in modules.values() for q in module['queries']]:
        ...
        union |= axioms
    record.update(status='PASS: fresh uniform-moment finite arithmetic, complete axiom audit',
                  axiom_union=sorted(union),
                  dependency_build_scope='Fresh compilation of the listed local module; pinned Mathlib dependency cache reused.',
