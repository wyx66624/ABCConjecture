#!/usr/bin/env python3
"""Fresh Actual pair-phase arithmetic and local dependency declarations.
...
SOURCES = {
    ...
    'DeepRootPhaseArithmetic': HERE / 'Lean/DeepRootPhaseArithmetic.lean',
}
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='deep-root-phase-fresh-', dir=scratch))
    modules = {}
    for name, source in SOURCES.items():
        ...
        modules[name] = dict(source=source.relative_to(ROOT).as_posix(),
                            ...
                            new=name == 'DeepRootPhaseArithmetic',
    ...
    record = dict(status='RUNNING: fresh source compilation not yet successful',
                  ...
                  modules=modules)
    result_path = out / 'phase_validation.json'
    ...
    for name, module in modules.items():
        ...
        if proc.returncode or 'sorryAx' in current or re.search(r'\berror:', current):
            (out / 'phase-lean.log').write_bytes(('\n'.join(logs)).encode())
    ...
    log = '\n'.join(logs)
    (out / 'phase-lean.log').write_bytes(log.encode())
    union = set()
    for query in [q for module in modules.values() for q in module['queries']]:
        ...
        union |= axioms
    record.update(status='PASS: fresh actual pair-phase arithmetic and local dependencies, complete axiom audit',
                  axiom_union=sorted(union),
                  dependency_build_scope='Fresh compilation of both listed local modules; pinned Mathlib dependency cache reused.',
                  not_claimed=['ABC proof or disproof', 'whole repository or Mathlib rebuild',
                               'finite-field torsion or lifting', 'Brun--Titchmarsh or divisor asymptotics',
