#!/usr/bin/env python3
"""Fresh 9-theorem actual-global bridge and 24 local dependency declarations.
...
SOURCES = {
    'ActualPrimeLogCompensation': ROOT / 'research/checkpoints/2026_09_07_depth_compensation/Lean/ActualPrimeLogCompensation.lean',
    'ActualPrimeLogHeight': ROOT / 'research/checkpoints/2026_09_07_depth_compensation/Lean/ActualPrimeLogHeight.lean',
    'ActualGlobalSignedBridge': HERE / 'Lean/ActualGlobalSignedBridge.lean',
}
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='actual-global-log-fresh-', dir=scratch))
    modules = {}
    for name, source in SOURCES.items():
        ...
        modules[name] = dict(source=source.relative_to(ROOT).as_posix(),
                            ...
                            new=name == 'ActualGlobalSignedBridge',
    ...
    record = dict(status='RUNNING: fresh source compilation not yet successful',
                  ...
                  modules=modules)
    result_path = out / 'global_log_validation.json'
    ...
    for name, module in modules.items():
        ...
        if proc.returncode or 'sorryAx' in current or re.search(r'\berror:', current):
            (out / 'global-log-lean.log').write_bytes(('\n'.join(logs)).encode())
    ...
    log = '\n'.join(logs)
    (out / 'global-log-lean.log').write_bytes(log.encode())
    union = set()
    for query in [q for module in modules.values() for q in module['queries']]:
        ...
        union |= axioms
    record.update(status='PASS: fresh actual-global bridge and local dependencies, complete axiom audit',
                  axiom_union=sorted(union),
                  dependency_build_scope='Fresh compilation of all three listed local modules; pinned Mathlib dependency cache reused.',
                  not_claimed=['ABC proof or disproof', 'whole repository or Mathlib rebuild',
                               'analytic full-mass estimate', 'actual tail membership or exceptional-root control'])
    result_path.write_bytes((json.dumps(record, indent=2)+'\n').encode())
