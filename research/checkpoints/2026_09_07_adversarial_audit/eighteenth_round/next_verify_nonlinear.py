#!/usr/bin/env python3
"""Fresh one-module NP1/CU1 audit, reusing an exact existing Mathlib cache.
...
def main():
    ...
    out = HERE / 'next_verification' / ('nonlinear-' + stamp)
    ...
    try:
        ...
        build = Path(tempfile.mkdtemp(prefix='abc-nonlinear-selectors-fresh-'))
        shutil.copyfile(cache / 'lean-toolchain', build / 'lean-toolchain')
        (build / 'lakefile.toml').write_text(
            'name = "ABCNonlinearSelectorAudit"\nversion = "0.1.0"\n\n'
            '[[require]]\nname = "mathlib"\n'
        ...
        needed.add('mathlib')
        manifest['name'] = 'ABCNonlinearSelectorAudit'
        ...
        source = HERE / 'next_Lean/NonlinearPowerSelectors.lean'
        ...
        record.update(status='RUNNING', compiler=compiler, mathlib_commit=rev,
                      ...
                      scope_sha256=digest((HERE / 'next_nonlinear_formal_scope.md').read_bytes()),
        ...
        record.update(status='PASS', axiom_union=sorted(union), theorem_axioms=audit,
                      ...
                      excluded=['CRT and prescribed valuations', 'irreducibility and separability',
                                'squarefree density and sieve', 'infinite tail bounds', 'ABC'])
        save()
