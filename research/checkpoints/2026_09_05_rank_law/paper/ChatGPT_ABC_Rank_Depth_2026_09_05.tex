...
\title{First-Appearance Depth and Radical Growth in an Eisenstein Orbit: Integrated Proof Edition}
...
\begin{theorem}[Complete local laws]\label{thm:local}
...
\]
Every product is over the actual finite prime support of $U_n$; $u_+=\max(u,0)$. Theorem~\ref{thm:local} implies $\mathcal L_n\mid n$ and
\[
...
\begin{theorem}[Elementary depth-two absorption]\label{thm:two}
For every $n\ge1$,
\[
...
\begin{theorem}[Depth-three absorption]\label{thm:three}
...
\end{theorem}
\begin{proof}
Since $|\alpha|=|\beta|=7^{3/2}$ and $|\alpha-\beta|=18\sqrt3$, the Binet formula and Lemma~\ref{lem:angle} give
\[
...
\begin{corollary}[Uniform restricted class]\label{cor:class}
...
\]
None is scalar, so none has $U$-value zero modulo $p$. The rank divides the full index by Theorem~\ref{thm:local}; any proper divisor divides one of the four tested exponents, so its exact value is 12876.
...
\bibitem{prior} ChatGPT, \emph{First-Appearance Depth and Radical Growth in an Eisenstein Orbit}, September 5, 2026; conversation source identified in the provenance paragraph. This edition consolidates proofs and does not repeat historical unconfirmed execution claims.
\end{thebibliography}
\end{document}
