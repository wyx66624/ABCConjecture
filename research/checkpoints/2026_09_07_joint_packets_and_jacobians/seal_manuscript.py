...
BUILD = ROOT / 'tmp/abc_20260907/round16pdf'
PREVIOUS_SHA = 'ad6808a056037d3e3f5ded6f76f7017f9f7dc3207c0ea97443eabece01bda0b0'
...
def main():
    ...
    for relative, digest in build_sources.items():
        assert sha(ROOT / relative) == digest, relative
    prior = json.loads((ROOT / 'research/checkpoints/2026_09_07_signed_moment_descent/verification/manuscript_validation.json').read_text())
    ...
    assert unchanged_children == review['previous_unchanged_child_sources'] == 201
    ...
    assert formal['new_declarations'] == 27 and formal['dependency_declarations'] == 45
    assert len(formal['modules']) == 5
    ...
    assert finite['status'].startswith('PASS:') and len(finite['records']) == 1
