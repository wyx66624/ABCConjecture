...
\hypersetup{pdftitle={Eisenstein Descent and Exact Boundary Divisibility},pdfauthor={ChatGPT},pdfsubject={Partial abc research; no unconditional proof},pdfkeywords={abc conjecture, Eisenstein descent, Lean, radical}}
\usepackage{enumitem}
\setlist{nosep}
...
\title{Eisenstein Descent and Exact Boundary Divisibility:\\
A Coupled Obstruction to Radical Estimates for \textit{abc}}
\author{ChatGPT}
\date{September 5, 2026 (America/Chicago)}
\begin{document}
\maketitle
\begin{abstract}
We continue an arithmetic research program on the standard abc inequality, without claiming a proof or disproof of that inequality. In the quadratic ring $\ZZ[\rho]$, $\rho^2=\rho-1$, we use the norm $N(x,y)=x^2+xy+y^2$ and the boundary form $F(x,y)=xy(x+y)$. An elementary Euclidean argument gives a prime-norm descent for every primitive pair: at each nonterminal step the associated height decreases by a factor at most $2/3$. This removes a coverage restriction present in common-power descent, but does not control the radical loss. Our central identity is the full, multiplicity-sensitive formula
\[
\gcdabs{F(w)}{F(\beta w)}=\gcdabs{F(w)}{F(\beta)}
\]
for every primitive $w$ and every integral $\beta$. On the orbit $(2+\rho)^n$, it yields strong divisibility of the entire boundary sequence, which is $6$ times the absolute value of the Lucas sequence with parameters $(20,343)$. We also construct normalized signed cubic triples for which the whole quadratic quotient is $7^n$ and its first-appearance excess has quadratic size in the base height. An explicit orbit point shows that every available prime-norm predecessor can worsen the normalized radical inequality. Forty named Lean theorems, including the general boundary gcd identity and strong divisibility, have passed a standalone compiler and axiom audit. Euclidean coverage and the asymptotic interpretations are ordinary mathematical proofs, not fully formalized here. The missing uniform radical estimate is stated explicitly.
\end{abstract}
...
The inspected project checkpoint \cite{project} contains several distinct lines: proper-face transport, compensated divisor packets, first-apparition estimates, arithmetic derivatives, and IUT/geometric interfaces. Earlier power descent compares a triple with a common-power lift and keeps the entire first-appearance factor separate from the controlled repeated-lifting factor \cite{power}. Earlier transverse lifting controls integer-realization overhead, but its proposed uniform small-norm condition was proved equivalent to abc \cite{transverse}. We do not assume either missing estimate.
...
The source snapshot is commit \texttt{9edd965d6df645dbebc0869530f3f5a40fb8cddc}. Its endpoint-residue obstruction \cite{endpoint} is retained: no positive-face existence assumption is used here. This manuscript develops its mathematical arguments directly. It does not assert priority relative to the external literature or external peer review.
...
\begin{align}
(u,v)(x,y)&=(ux-vy,\ vx+(u+v)y),\label{eq:mul}\\
\overline{(x,y)}&=(x+y,-y),\\
N(x,y)&=x^2+xy+y^2.
\end{align}
...
\begin{lemma}[Normalization and height corridor]\label{lem:height}
...
\begin{lemma}[Coprime norm and boundary]\label{lem:disjoint}
...
\begin{lemma}[Euclidean division]\label{lem:euclidean}
...
\begin{theorem}[Every primitive pair has a prime-norm predecessor]\label{thm:coverage}
...
This proof does not assume that either arm is an integer power. It also does not assert that a radical inequality improves along the path. The latter statement is false, even when all prime-norm predecessor choices are allowed; see Section~\ref{sec:counter}.

\section{The exact boundary gcd identity}
\begin{theorem}[Exact boundary overlap]\label{thm:gcd}
...
\end{theorem}
\begin{proof}
Put $A=F(u,v)$ and $T=F((u,v)(x,y))$. Formula~\eqref{eq:mul} gives the exact congruences
\begin{align}
T&\equiv-Ay^3\pmod{x},\label{eq:remx}\\
T&\equiv Ax^3\pmod{y},\\
...
\[
\gcdabs{x}{T}=\gcdabs{x}{A},\quad
\gcdabs{y}{T}=\gcdabs{y}{A},\quad
...
For a nonterminal edge $z=\pi w$ from Theorem~\ref{thm:coverage}, with $w$ nonunit, the common radical factor is
\[
...
\begin{lemma}[Primitivity of the entire orbit]\label{lem:primitive}
...
\end{lemma}
\begin{proof}
The congruences hold at $n=1$. Under~\eqref{eq:step}, their difference becomes a multiple of seven, and $y_{n+1}\equiv5y_n\pmod7$, so they persist. A common divisor of $x_n,y_n$ divides their norm $7^n$, but is coprime to seven by the second congruence. It is therefore one. The case $n=0$ is immediate.
\end{proof}
...
\begin{theorem}[A whole-boundary strong divisibility sequence]\label{thm:strong}
For all $m,n\in\NN$,
...
\end{theorem}
\begin{proof}
The orbit multiplication law and Theorem~\ref{thm:gcd} give
\[
\gcd(B_m,B_{m+n})=\gcd(B_m,B_n).
\]
Repeated subtraction, or Euclidean induction on $(m,n)$, proves~\eqref{eq:strong}; at an index zero it reduces to $\gcd(0,B_n)=B_n$.

For the sequence identity, direct polynomial expansion for the map $S(x,y)=(2x-y,x+3y)$ gives
\[
F(S^2z)=20F(Sz)-343F(z).
\]
Since $F_0=0$ and $F_1=6$, induction proves~\eqref{eq:lucas}. As an independent explanation, the coefficient of $\rho$ in $z^3$ is $3F(z)$, whereas $\alpha^3$ has trace $20$ and norm $343$.
\end{proof}
...
\begin{lemma}[A non-power subsequence]\label{lem:modfour}
For $n=6j+1$, $j\ge0$,
\[
...
\begin{theorem}[Quadratic first-appearance excess]\label{thm:quadratic}
...
\end{equation}
The resulting signed cubic relation gives a positive primitive abc triple, and the bases are normalized as in Lemma~\ref{lem:modfour}.
\end{theorem}
\begin{proof}
The polynomial identity $x^3-y^3=(x-y)(x^2+xy+y^2)$ proves the first equation. The residues from Lemma~\ref{lem:primitive} give $x/y=2$ modulo seven. This residue has order three, and neither $x$, $y$, nor $x-y$ is zero modulo seven. The exact valuation follows. In particular $x-y\ne0$; the other coordinates are also nonzero. Their pairwise coprimality follows from that of $x,y$.

For arbitrary real $x,y$ with $h=\max(|x|,|y|)$,
\[
\tfrac34 h^2\le x^2+xy+y^2\le3h^2.
\]
For the lower bound, fix the coordinate of absolute value $h$ and complete the square in the other coordinate. Divide these inequalities by seven and use $N=7^n$ to obtain~\eqref{eq:T}.
...
For every fixed $\theta<2$ and $C>0$, the normalized signed homogeneous family in Theorem~\ref{thm:quadratic} contains infinitely many members with $T>Ch^\theta$.
\end{corollary}
...
\begin{proposition}[The exact coupled radical still required]\label{prop:coupled}
...
\begin{equation}\label{eq:cubic-height}
...
\end{equation}
No proof of~\eqref{eq:restricted} is supplied here.
\end{proposition}
\begin{proof}
Equation~\eqref{eq:cubic} and $7\nmid xy(x-y)$ give the radical formula. If $x,y$ have the same sign, then $C_n=h^3$ and $h^2\le N\le3h^2$. If they have opposite signs, write $u=|x|$, $v=|y|$; then $C_n=(u+v)N$, with
$N=u^2-uv+v^2$ and $N\le(u+v)^2\le4N$. These prove~\eqref{eq:cubic-height}.

If the restricted abc statement holds for every $\varepsilon>0$, the lower height bound gives
$S_n\ge c_\varepsilon7^{3n/(2(1+\varepsilon))}$. For any $0<\delta<3/2$, choose $\varepsilon$ small enough that $3/(2(1+\varepsilon))\ge3/2-\delta$. Larger $\delta$ are immediate. Conversely, for a given $\varepsilon>0$, choose $0<\delta<3\varepsilon/(2(1+\varepsilon))$ and use~\eqref{eq:restricted} together with the upper height bound. The constants depend only on the chosen exponent, not on $n$.
\end{proof}
The sequence $S_n$ in this proposition is not $\rad(B_n)$. Substituting the strong-divisibility conclusion for the latter into~\eqref{eq:restricted} would confuse two different boundary forms.

\section{A complete obstruction to pointwise no-loss descent}\label{sec:counter}
\begin{theorem}[Every prime-norm predecessor can have smaller normalized defect]\label{thm:counter}
...
\]
The displayed factors are prime by trial division up to their square roots, and multiplication verifies the products. Thus $R_5=803706$ and $R_6=358530$. Inequality~\eqref{eq:counter} follows from $149<360$ and $358530<803706$. Its denominator-cleared form is formally proved for every $m\in\NN$.
...
\begin{proposition}[Adding both factors does not restore the same constant]\label{prop:both}
...
Theorem~\ref{thm:coverage} supplies actual predecessors for all triples; Theorem~\ref{thm:gcd} identifies their shared boundary divisors exactly; Theorem~\ref{thm:counter} prevents free pointwise monotonicity, and Proposition~\ref{prop:both} prevents a same-constant transfer from both factors. The unresolved task is to prove a quantitative estimate on new boundary primes and their multiplicities that, when accumulated, bounds $q_m$ independently of the final triple. No such estimate follows from the gcd identity alone. If the norm itself is prime, the predecessor is a unit and the multiplier is the original element; thus coverage alone has not reduced the height problem on that stratum.

There is also a useful separation within the fixed orbit. Lemma~\ref{lem:height} gives
\[
7^{n/2}\le H(x_n,y_n)\le\frac2{\sqrt3}7^{n/2}.
\]
The original boundary orbit would require
$\rad(6U_n)\ge c_\delta7^{(1/2-\delta)n}$ for every $\delta>0$; the signed cubic family instead requires~\eqref{eq:restricted}. Each is a restricted-family condition, not a resolution for arbitrary triples. Strong divisibility resolves support overlap across indices but not either radical growth condition.

The existing IUT same-pilot and all-place comparisons, compensated packet bounds, Mersenne depth-three counting, Pell first-apparition valuations, and geometric uniformity gates in \cite{project} remain unresolved in this work. Their parent routes are retained. The fixed nonreal Lucas sequence above is not identified with the earlier real Pell sequence. What is shared is the precise first-valuation bottleneck, not an unproved transfer of estimates.
...
\bibitem{project} ChatGPT and project contributors, \emph{ABC Conjecture Research and Lean Formalization}, repository \texttt{wyx66624/ABCConjecture}, inspected commit \texttt{9edd965d6df645dbebc0869530f3f5a40fb8cddc}. Entry points: \texttt{README.md} and \texttt{research/ABC\_ROUTE\_BOTTLENECKS.md}.
\bibitem{power} ChatGPT, \emph{Power-Radical Descent, Unbounded First-Appearance Depth, and Pairwise Lifting of Arithmetic Derivatives}, project checkpoint \texttt{2026\_09\_05\_power\_descent}, 2026. A partial-research source, not a proof of abc.
\bibitem{transverse} ChatGPT, \emph{Quantitative Lifting of Arithmetic Derivatives and the Transverse Obstruction to abc}, project checkpoint \texttt{2026\_09\_05\_transverse\_lifting}, 2026. A partial-research source.
\bibitem{endpoint} Project contributors, \emph{Endpoint residue packets: unbounded counting obstruction and explicit verification scope}, pull request 419, incorporated in the inspected snapshot, 2026.
\bibitem{lean} Lean contributors, \emph{Lean 4}, pinned release 4.32.2, compiler commit \texttt{f3b06c705e6c85f5314019d5d3baab0fec5b580c}. The formal certificate is the supplied source plus compiler and axiom-audit output, not this bibliographic entry.
\end{thebibliography}
\end{document}
