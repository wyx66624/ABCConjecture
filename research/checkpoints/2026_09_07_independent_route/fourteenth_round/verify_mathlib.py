"""Fresh scoped compilation of SS integer arithmetic against pinned Mathlib."""
...
def main():
    ...
    source = here/'Lean/SimultaneousEllipticArithmetic.lean'
    ...
    assert len(declarations) == 12 and set(declarations) == set(queries)
    ...
    with tempfile.TemporaryDirectory(prefix='abc-ss-fresh-') as fresh:
        fresh = Path(fresh)
    ...
    record = {'status': 'PASS', 'compiler': compiler, 'mathlib_commit': pin,
              ...
              'declarations': 12, 'axiom_queries': axioms,
              'fresh_temporary_source_and_olean': True,
              'warnings_as_errors': True,
              'scope': 'Actual SS polynomial, square divisibility and prime-depth arithmetic. Does not formalize curves, isogeny degree, Jacobians or rational-point completeness.'}
    data = (json.dumps(record, sort_keys=True, indent=2)+'\n').encode()
    (out/'mathlib_validation.json').write_bytes(data)
    print(json.dumps({'status': 'PASS', 'declarations': 12,
