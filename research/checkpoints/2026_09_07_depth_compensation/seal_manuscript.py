...
BUILD = ROOT / 'tmp/abc_20260907/round10pdf'
PREVIOUS_SHA = '41aef4881562d618fea63bc7593bc2e96658da079a9e7022b9d5110c41fd977a'
...
def main():
    ...
    for relative, digest in build_sources.items():
        assert sha(ROOT / relative) == digest, relative
    prior = json.loads((ROOT / 'research/checkpoints/2026_09_07_signed_compatibility/verification/manuscript_validation.json').read_text())
    ...
    assert unchanged_children == review['previous_unchanged_child_sources'] == 154
    ...
    assert formal['status'].startswith('PASS:') and formal['new_theorems'] == 17
    for entry in formal['modules'].values():
        assert sha(ROOT / entry['source']) == entry['sha256']
    prime_log = json.loads((HERE / 'verification/prime_log_validation.json').read_text())
    assert prime_log['status'].startswith('PASS:') and prime_log['declarations'] == 24
    for entry in prime_log['modules'].values():
        assert sha(ROOT / entry['source']) == entry['sha256']
    ...
    result = {'status': 'PASS: full manuscript compiled and changed pages visually reviewed',
              ...
              'formal_records': ['verification/lean_validation.json', 'verification/prime_log_validation.json'],
