#!/usr/bin/env python3
"""Fresh, scoped Std-only compilation of boundary support arithmetic."""
...
MODULES = {
    ...
    'BoundarySupportArithmetic': '2026_09_07_cm_image',
}
...
def main():
    ...
    build = Path(tempfile.mkdtemp(prefix='cm-image-fresh-', dir=scratch))
    ...
    (build / 'lakefile.toml').write_text(
        'name = "ABCBoundarySupportAudit"\nversion = "0.1.0"\n'
        'defaultTargets = ["BoundarySupportAudit"]\n[leanOptions]\nwarningAsError = true\n'
        '[[lean_lib]]\nname = "BoundarySupportAudit"\nroots = [' + roots + ']\n')
    inventory = {}
    for name, checkpoint in MODULES.items():
        ...
        inventory[name] = {
            ...
            'new': name == 'BoundarySupportArithmetic',
        }
    ...
    result = {
        ...
        'not_claimed': ['ABC proof or disproof', 'full repository build',
                        ...
                        'complete eighth-round formalization', 'Hensel or Faltings theorem', 'finite Kummer descent', 'Tate algorithm or modularity', 'external peer review'],
    }
