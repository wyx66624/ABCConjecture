...
The supplemental `SteinbergIntegerFiniteChain20260902` module now proves the
integer exponent-gcd construction, the actual finite-support cell norm, and
the concrete filling inequality under an explicit exact surface-boundary
premise.  The inductive relation generated by the permitted rational
five-term moves and the theorem that generated chains satisfy that premise
remain open.  The Lean Pythagorean-square theorems close both single-cell
Gates MC and SC, and no arbitrary multi-cell gate.
