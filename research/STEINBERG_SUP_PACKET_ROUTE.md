...
## 3. Exact recovery by the nonarchimedean sup norm

Equip the direct sum of the cyclic-line determinant coordinates with the
nonarchimedean sup norm.  On logarithmic norms this applies the maximum.
...
The operation in (4) is invariant under every permutation of the cyclic
...
At an archimedean place use the Euclidean norm on the `ell+1` coordinates.  Its
logarithmic value for the two-valued packet is
...
Taking one half of the logarithm proves (5).
...
## 5. Descent as a Galois-invariant vector packet
...
\[
 \mathcal V_\ell
 =\bigoplus_{C\in\mathcal L_\ell}\mathcal M_C,
\]

where `M_C` is the cyclic-line evaluation or determinant line.  Galois permutes
the summands.  The tuple of all conjugate evaluation sections is therefore a
rational section of the descended permutation bundle.  The finite sup norm and
the archimedean Euclidean norm are invariant under this permutation action.
...
Galois-invariant vector whose local norm is nonlinear.

## 6. Arakelov maximal slope rather than total degree

Let `sigma_ell` be the descended packet section.  The line generated by this
vector satisfies the standard slope inequality
...
A direct sum has rank `ell+1`, but (6) involves the **maximal slope**, not the
...
`log ell=o(log c)`, (8) implies the logarithmic abc inequality.

## 7. The exact remaining theorem

The route is reduced to the following classical, falsifiable statement.

### Target theorem 7.1 (Steinberg packet slope theorem)

Construct the descended metrized packet
`V_ell` and its rational section `sigma_ell` so that:

1. at a multiplicative place its coordinate norms are exactly the cyclic-line
   Neron energies in (1);
2. good finite places have sup norm one;
...
4. the archimedean metric differs from the sup metric by at most
   `log(ell+1)/2`;
5. the maximal slope bound (7) holds with every metric and Jacobian explicitly
   normalized.
...
Bost slope inequalities, or the modular interpretation of the Steinberg
representation.
...
line.  The sup packet uses the complete orbit, is Galois invariant, and applies
a nonlinear norm before global summation.

A future counterexample would have to show that no descended packet can satisfy
Target theorem 7.1, or that its maximal slope necessarily carries a coefficient
strictly larger than `(ell-1)/2` by a nonvanishing proportion.  No such theorem
is presently known, so the route remains active.
