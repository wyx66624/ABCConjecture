...
\section*{Integrated checkpoint: Power-radical descent and first-depth obstructions}
\phantomsection\label{checkpoint:power}
\addcontentsline{toc}{section}{Integrated checkpoint: Power-radical descent and first-depth obstructions}
...
The target is
\begin{equation}\label{power:eq:abc}
...
\eqref{power:eq:abc} is used as a premise in our unconditional results.
...
\cite{power:transverse} controls integer realization of arithmetic derivatives,
...
\cite{power:multiflag} separates repeated lifting from first appearance for
...
\begin{equation}\label{power:eq:cleared}
\Qm(a,b,c)=\frac{c^m}{\rad(abc)^{m+1}}.
\end{equation}
Uniform boundedness of \eqref{power:eq:cleared} for every positive integer $m$ is
equivalent to \eqref{power:eq:abc}. Indeed, abc with $\varepsilon=1/m$ gives the
...
\begin{lemma}[Elementary valuation lifting]\label{power:lem:lte}
...
&L_n(x,y)&=\prod_{p\mid b_n}p^{v_p(n)},\label{power:eq:TL}\\
E_n(x,y)&=
\begin{cases}
2^{v_2(x+y)-1},&x,y\text{ odd and }n\text{ even},\\
1,&\text{otherwise},
\end{cases}
&B_n(x,y)&=E_nL_nT_n.\label{power:eq:B}
\end{align}
Empty products are one. Notice that $L_n\mid n$.

\begin{theorem}[Exact relative radical identity]\label{power:thm:balance}
For every $n\ge1$,
\begin{equation}\label{power:eq:balance}
R_n B_n=R_1 g_n.
\end{equation}
...
in common with $xy$. For an odd old prime, Lemma~\ref{power:lem:lte} gives
...
\eqref{power:eq:B}. These identities exhaust the prime divisors of $g_n$.
\end{proof}
...
\eqref{power:eq:TL} with the new seed and sum. Set $B_n^+=L_n^+T_n^+$.

\begin{proposition}\label{power:prop:sum}
For odd $n$, $L_n^+\mid n$ and
\begin{equation}\label{power:eq:sumbalance}
R_n^+B_n^+=R_1^+g_n^+.
\end{equation}
\end{proposition}
\begin{proof}
For a new prime, apply the odd-sum part of Lemma~\ref{power:lem:lte} at $d_p$ and
...
\begin{theorem}[Exact defect transfer]\label{power:thm:transfer}
For difference powers,
\begin{equation}\label{power:eq:minusdefect}
\frac{\Qm^-(x,y;n)}{\Qm^-(x,y;1)}
=\frac{x^{m(n-1)}B_n^{m+1}}{g_n^{m+1}}
\le\frac{B_n^{m+1}}{x^{n-1}}.
\end{equation}
For odd sum powers,
\begin{equation}\label{power:eq:plusdefect}
\frac{\Qm^+(x,y;n)}{\Qm^+(x,y;1)}
=\frac{(B_n^+)^{m+1}}{g_n^+}.
\end{equation}
...
\begin{corollary}[An obstruction in a minimal counterexample]\label{power:cor:descent}
...
\begin{equation}\label{power:eq:criticalT}
...
at most $K$. Apply Theorem~\ref{power:thm:transfer} contrapositively. For odd
...
supply a predecessor for every triple. Furthermore, \eqref{power:eq:criticalT}
...
\begin{proposition}[Complete-budget composition]\label{power:prop:cocycle}
...
\begin{equation}\label{power:eq:rebase}
...
$(X,Y)$ is not a simultaneous nontrivial power. Formula~\eqref{power:eq:rebase}
...
\begin{theorem}\label{power:thm:stratum}
...
\begin{lemma}[A polynomial lifting orbit]\label{power:lem:orbit}
Set
\[
...
\begin{equation}\label{power:eq:orbitpoly}
\phi(X+32\phi(X))=\phi(X)(1024X^2+1088X+1057).
\end{equation}
At $X\equiv30\pmod{49}$, $\phi(X)\equiv0\pmod{49}$, so the first
residue is preserved. The multiplier in \eqref{power:eq:orbitpoly} is $42$ modulo
...
\begin{theorem}[Small normalized representatives]\label{power:thm:small}
...
\begin{align}
2\le x_k&<28\cdot7^{k+1},&x_k&\equiv2\pmod4,\quad x_k\equiv2\pmod7,\label{power:eq:smallsize}\\
7^{k+2}&\mid\phi(x_k),& 7^{k+2}&\le\phi(x_k).\label{power:eq:smalldiv}
\end{align}
...
nonnegative integers, \eqref{power:eq:smalldiv} implies $x_k\to\infty$.
The sequence of small representatives need not be strictly increasing;
no monotonicity of these remainders is used.
\end{proof}

\begin{corollary}[No independent sublinear first-depth bound]\label{power:cor:linear}
There are infinitely many integers $x$ that are not nontrivial perfect
powers and satisfy
\begin{equation}\label{power:eq:lineart}
T_3(x,1)>x/28.
\end{equation}
...
This implementation does not use the recurrence \eqref{power:eq:orbitpoly}.
...
radical from above strongly enough to contradict \eqref{power:eq:abc}.
...
\begin{lemma}[The attainable-value ideal]\label{power:lem:ideal}
...
\begin{theorem}[Pair-energy proximity]\label{power:thm:energy}
For every attainable integer $d$ there is an integer vector with
$\partial_\lambda(N)=d$ and
\begin{equation}\label{power:eq:kap}
...
\]
A feasible vector exists by Lemma~\ref{power:lem:ideal}; each bounded sublevel
...
\le\frac1{2S}\sum_{j\ne i}f_j(f_i+f_j)v,
\]
proving \eqref{power:eq:kap}. Finally $v\le1$ and
$(f_i+f_j)/2\le\max_i f_i$; sum the weights and divide by $S$.
The two special cases follow by direct substitution.
\end{proof}

\begin{corollary}[A terminating pair-move algorithm]\label{power:cor:algorithm}
...
\eqref{power:eq:kap}. No polynomial bound on the number of moves is asserted.
\end{corollary}
\begin{proof}
...
for every pair. The proof of Theorem~\ref{power:thm:energy} only needs these
inequalities, not global optimality.
\end{proof}

The previous local bound was $S=\Omega(N)/k$ \cite{power:transverse}. The new
...
\]
Replacing its local lifting by Theorem~\ref{power:thm:energy} changes the overhead
to at most
\[
...
Warnings are treated as errors. The axiom union is
...
appearance and the seed's radical or defect. Corollary~\ref{power:cor:linear}
...
\eqref{power:eq:abc} is still unresolved. No completed abc proof, disproof, or
