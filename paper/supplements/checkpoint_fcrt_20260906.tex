...
\section*{Integrated checkpoint: Flagged CRT obstructions and exact divisor gaps}
\phantomsection\label{checkpoint:fcrt}
\addcontentsline{toc}{section}{Integrated checkpoint: Flagged CRT obstructions and exact divisor gaps}
...
\cite{fcrt:repoendpoint,fcrt:repoanchor,fcrt:repoaudit} are the immediate antecedents.
...
\begin{equation}\label{fcrt:eq:block}
...
\begin{equation}\label{fcrt:eq:flag}
...
\begin{equation}\label{fcrt:eq:height}
...
\begin{equation}\label{fcrt:eq:creditfactor}
 f_\nu=\min\{Q_{T_\nu}/D_{S_\nu},Q_{U_\nu}\};
\end{equation}
...
\begin{theorem}[Arithmetic once-charge certificate]\label{fcrt:thm:certificate}
Every configuration satisfying the actual disjointness, saturation, owner,
and face conditions above satisfies
\begin{equation}\label{fcrt:eq:rationalbridge} c\le R F.\end{equation}
...
at least two. Substitute \eqref{fcrt:eq:height} and multiply by \(\rad(c)\).
...
independent audit \cite{fcrt:repoaudit}.
\end{remark}
...
\begin{equation}\label{fcrt:eq:z}
 z_p(T)=\min\{Q_D:\varnothing\ne D\subseteq T,
                         \ \Phi(D)=-\Phi(T)\},
\end{equation}
with value \(+\infty\) when the set is empty.

\begin{theorem}[Exact optimal-face normal form]\label{fcrt:thm:normal}
...
\(Q_D\). Monotonicity of the minimum in \eqref{fcrt:eq:creditfactor} gives
\eqref{fcrt:eq:optimalfactor}; cancellation gives \eqref{fcrt:eq:loss}.
\end{proof}

When \(\Phi(T)=-1\), the deletion label in \eqref{fcrt:eq:z} is one. For a
...
\begin{proposition}\label{fcrt:prop:cost}
...
Theorem~\ref{fcrt:thm:normal}. Substitute this inequality.
\end{proof}
...
\begin{lemma}[Bilinear factor gap]\label{fcrt:lem:gap}
...
\begin{theorem}[Prime-power no-face threshold]\label{fcrt:thm:noface}
...
\begin{equation}\label{fcrt:eq:threshold} c\ge M(2M-1).\end{equation}
...
\(C+t=s(Mt+1)\). Lemma~\ref{fcrt:lem:gap} yields \eqref{fcrt:eq:threshold}.
\end{proof}
...
\begin{corollary}[Balanced two-source endpoints]\label{fcrt:cor:balanced}
Suppose \(c=2^e3^f\), \(e,f\ge2\), and
\begin{equation}\label{fcrt:eq:balanced}
...
is less than \(2M-1\), by \eqref{fcrt:eq:balanced}. Apply
Theorem~\ref{fcrt:thm:noface}. A proper face of any block is also a proper face
...
\begin{theorem}\label{fcrt:thm:infinite}
There are infinitely many distinct primitive triples \((1,c-1,c)\) such
that \(c=2^e3^f\), \eqref{fcrt:eq:balanced} holds, \(49\mid c-1\), and
\begin{equation}\label{fcrt:eq:hit}
...
so \eqref{fcrt:eq:balanced} holds eventually. The elementary congruences
...
\eqref{fcrt:eq:hit}. Select an increasing subsequence of \(c\) to ensure
distinctness, and invoke Corollary~\ref{fcrt:cor:balanced}.
\end{proof}


\begin{corollary}[Unbounded absolute defect without flags]\label{fcrt:cor:unbounded}
...
Theorem~\ref{fcrt:thm:infinite}, replace the constants \(21,42\) by
...
Theorem~\ref{fcrt:thm:infinite} refutes the following proposed shortcut, including
its ``all sufficiently large'' variant: every positive-defect two-source
unit-gap endpoint has at least one proper compatible face. Corollary~\ref{fcrt:cor:unbounded}
...
anchored-prefix theorem \cite{fcrt:repoanchor}; that theorem has additional
...
Let the hypotheses of Corollary~\ref{fcrt:cor:balanced} hold. Write
\[
 Q=\rad(c-1),\quad A=2^{e-1},\quad B=3^{f-1},\quad D=\log(AB/Q).
\]
In this section assume \(AB>Q\), so \(D=\Delta>0\).

\begin{theorem}[Exact reduction on the no-flag stratum]\label{fcrt:thm:reduction}
At these endpoints,
\begin{equation}\label{fcrt:eq:allopt}
...
\begin{equation}\label{fcrt:eq:distance}
...
contradict Corollary~\ref{fcrt:cor:balanced}. Disjointness permits at most one
...
This proves \eqref{fcrt:eq:allopt}. To obtain \eqref{fcrt:eq:distance}, put
...
study of dense divisors \cite{fcrt:weingartner}. We include the elementary
...
\begin{lemma}\label{fcrt:lem:dense}
...
\begin{corollary}\label{fcrt:cor:densebound}
...
most \((g-D)/2\) from the interval. Apply Theorem~\ref{fcrt:thm:reduction}.
\end{proof}
...
\cite{fcrt:iut}. Inspection of those passages does not supply the missing
...
Joshi's report \cite{fcrt:joshi} takes a distinct position: it criticizes both
...
\cite{fcrt:bae} is an elementary sufficient selection principle. Its proof
...
endpoint. Theorem~\ref{fcrt:thm:infinite} exhibits infinitely many endpoints
where any sufficient condition implying a proper flag must fail.

Lichtman's discussion of the almost-all \(abc\) result \cite{fcrt:lichtman}
...
\cite{fcrt:dujella} controls a different Diophantine configuration. No uniform
...
\cite{fcrt:lean321,fcrt:lean322}; the latter also warns that the affected checking path
...
\begin{equation}\label{fcrt:eq:uniform}
 \forall\varepsilon>0\ \exists C_\varepsilon\in\R\
 \forall (a,b,c),\qquad
 B_{\FCRT}(a,b,c)\le\varepsilon\log\rad(abc)+C_\varepsilon.
\end{equation}
Together with Theorem~\ref{fcrt:thm:certificate}, it gives the usual
\(c\le K_\varepsilon\rad(abc)^{1+\varepsilon}\).
Neither \eqref{fcrt:eq:uniform} nor its negation is established.
On the positive-defect no-flag stratum, Theorem~\ref{fcrt:thm:reduction} says
